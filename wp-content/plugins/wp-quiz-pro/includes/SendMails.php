...
class SendMails {
	...
	public function init() {
		...
		add_action( 'wp_quiz_after_track_player', array( $this, 'send_admin_result_email' ), 10, 4 );
	}
	...
	public function send_quiz_result_email( array $subscribe_data, Quiz $quiz ) {
		...
		$replaces = array(
			...
			'%%quiz_url%%'         => $subscribe_data['current_url'],
	 ...
	 * @param Quiz  $quiz         Quiz object.
	 */
	public function send_admin_result_email( $play_data_id, array $insert_data, array $request_data, Quiz $quiz ) {
		...
		error_log( print_r( $request_data['current_url'], true ) );

		$replaces = array(
			'%%quiz_url%%'    => $request_data['current_url'],
			'%%quiz_name%%'   => $quiz->get_title(),
			'%%quiz_result%%' => $quiz_result,
		);
